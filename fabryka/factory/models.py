...
class Uczen(models.Model):
    ...
    def __str__(self):
        s = str(self.imie_uczen) + ' ' + str(self.nazwisko_uczen) + ' ' + str(self.indeks_uczen)
        return s
...
class Opiekun(models.Model):
    ...
    def __str__(self):
        s = str(self.imie_opiekun) + ' ' + str(self.nazwisko_opiekun) + ' ' + str(self.indeks_opiekun)
        return s
...
class ListaPrac(models.Model):
    ...
    uczen_praca = models.ForeignKey(Uczen, on_delete=models.CASCADE, blank=True, null=True)
    termin_praca = models.DateTimeField(default=datetime.now, blank=True)
    opiekun_praca = models.ForeignKey(Opiekun, on_delete=models.CASCADE, blank=True, null=True)

    def __str__(self):
        s = str(self.temat_praca) + ' ' + str(self.uczen_praca) + ' ' + str(self.termin_praca)
        return s
